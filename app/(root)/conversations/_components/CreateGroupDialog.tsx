...
const createGroupFormSchema = z.object({
    ...
    console.log(friends, unselectedFriends, members)
    ...
    return <Dialog>
        <Tooltip>
            <TooltipTrigger asChild>
                <DialogTrigger asChild>
                    <Button size="icon" variant="outline" className="rounded-2xl w-10 h-10">
        ...
        <DialogContent>
            <DialogHeader>
                <DialogTitle className='font-black text-3xl text-slate-500'>Create Group</DialogTitle>
                <DialogDescription className='text-slate-500'>Add your friend to get started</DialogDescription>
            </DialogHeader>
            <Form {...form}>
                <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-8'>
                    <FormField control={form.control} name='name' render={({field}) => {
                        return <FormItem>
                            ...
                            <FormControl>
                                <Input placeholder='Group name...' {...field} className='rounded-xl'></Input>
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    }} />
                    ...
                    ? <Card className='flex items-center gap-3 overflow-x-auto w-full h-24 p-2 no-scrollbar'>
                        {friends?.filter((friends : any) => members.includes(friends._id)).map((friend : any) => {
                            return <div key={friend._id} className='flex flex-col items-center gap-1'>
                                <div className='relative'>
                                    ...
                                    <X className='text-muted-foregroud w-4 h-4 absolute 
                                    bottom-8 left-7 bg-muted rounded-full cursor-pointer' 
                    ...
                    : null }
                    <DialogFooter>
                        <Button disabled={pending} type='submit' className='rounded-xl'>Create</Button>
                    </DialogFooter>
                </form>
            </Form>
        </DialogContent>
